...
async function downloadWithYtDlp(url, quality, outputPath) {
    ...
    if (quality === "360p") formatStr = "bestvideo[height<=360]+bestaudio/best[height<=360]";
    else if (quality === "480p") formatStr = "bestvideo[height<=480]+bestaudio/best[height<=480]";
    else if (quality === "720p") formatStr = "bestvideo[height<=720]+bestaudio/best[height<=720]";
    else if (quality === "1080p") formatStr = "bestvideo[height<=1080]+bestaudio/best[height<=1080]";
    ...
    // Add --recode-video to ensure proper MP4 format that plays everywhere
    const cmd = isWindows
        ? `"${YT_DLP}" --no-playlist -f "${formatStr}" --merge-output-format mp4 --recode-video mp4 --no-check-certificate --user-agent "Mozilla/5.0" --ffmpeg-location "${FFMPEG}" -o "${outputPath}" "${url}"`
        : `${YT_DLP} --no-playlist -f "${formatStr}" --merge-output-format mp4 --recode-video mp4 --no-check-certificate --user-agent "Mozilla/5.0" -o "${outputPath}" "${url}"`;
