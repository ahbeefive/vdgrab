...
// Paths to executables (use system binaries - installed via Docker/package manager)
const isWindows = process.platform === 'win32';
const YT_DLP = "yt-dlp";  // System yt-dlp (installed via pip)
const FFMPEG = "ffmpeg";   // System ffmpeg (installed via apt)

console.log('🔧 Using yt-dlp from system PATH');
...
app.post("/api/download", async (req, res) => {
    try {
        ...
        try {
            ...
            console.log("⚠️ Could not get title, using default");
            cleanTitle = `${platform}_video_${Date.now()}`;
        }
        ...
        // Video Download - Platform-specific handling
        ...
        if (platform === "youtube") {
            // YouTube: Use quality-specific formats with proper merging (NO RECODE for speed)
            if (quality === "360p") {
            ...
            // TikTok: Download best quality, no watermark
            downloadCmd = isWindows
                ? `"${YT_DLP}" --no-playlist --no-check-certificate --user-agent "Mozilla/5.0" --referer "https://www.tiktok.com/" --ffmpeg-location "${FFMPEG}" -o "${outputPath}" "${url}"`
                : `${YT_DLP} --no-playlist --no-check-certificate --user-agent "Mozilla/5.0" --referer "https://www.tiktok.com/" -o "${outputPath}" "${url}"`;
                
        } else if (platform === "facebook") {
            // Facebook: Use best available format
            downloadCmd = isWindows
                ? `"${YT_DLP}" --no-playlist --no-check-certificate --user-agent "Mozilla/5.0" --ffmpeg-location "${FFMPEG}" -o "${outputPath}" "${url}"`
                : `${YT_DLP} --no-playlist --no-check-certificate --user-agent "Mozilla/5.0" -o "${outputPath}" "${url}"`;
        ...
        console.log(`🎥 Downloading ${platform} ${quality}...`);
        ...
        res.download(outputPath, `${cleanTitle}.mp4`, (err) => {
            ...
            if (err) console.error("❌ Error:", err);
            else console.log(`✅ ${platform} ${quality} sent`);
        });
        ...
        if (!res.headersSent) {
            res.status(500).json({ 
                ...
                suggestion: "Try a different video or check if the URL is valid"
            });
        }
    }
});
